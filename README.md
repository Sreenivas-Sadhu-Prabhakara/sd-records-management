...
| **Asset Type** | Record Archive |
| **Control Record** | Record Archive Maintenance Agreement |
...
| POST | `/v1/record-archive-maintenance-agreement/initiate` | Initiate |
| GET | `/v1/record-archive-maintenance-agreement` | Retrieve (list) |
| GET | `/v1/record-archive-maintenance-agreement/{crId}/retrieve` | Retrieve |
| PUT | `/v1/record-archive-maintenance-agreement/{crId}/update` | Update |
| PUT | `/v1/record-archive-maintenance-agreement/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/record-archive-maintenance-agreement/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-records-management:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-records-management ./helm -n bian-risk-compliance
```

Exposed through the platform Gateway at path prefix `/sd-records-management` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
